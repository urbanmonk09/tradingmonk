...
export function detectFairValueGap(highs: number[], lows: number[]): boolean {
  if (!highs?.length || !lows?.length || highs.length < 3 || lows.length < 3) return false;
  const i = highs.length - 3;
  const prevHigh = highs[i];
  const nextLow = lows[i + 2];
  return isFinite(prevHigh) && isFinite(nextLow) && prevHigh !== 0
    ? Math.abs(nextLow - prevHigh) / Math.abs(prevHigh) > 0.005
    : false;
}
...
export function detectVolumeSurge(volumes: number[]): boolean {
  ...
  return isFinite(latest) && isFinite(avg) ? latest > avg * 1.5 : false;
}

export function detectLiquiditySweep(highs: number[], lows: number[], current: number): "BULLISH" | "BEARISH" | null {
  if (!highs?.length || !lows?.length || highs.length < 5 || lows.length < 5) return null;
  const recentHigh = Math.max(...highs.slice(-6, -1));
  const recentLow = Math.min(...lows.slice(-6, -1));
  const penHigh = highs[highs.length - 2];
  const penLow = lows[lows.length - 2];
  if (!isFinite(current) || !isFinite(recentHigh) || !isFinite(recentLow)) return null;
  if (current > recentHigh * 1.001 && current < penHigh) return "BEARISH";
  if (current < recentLow * 0.999 && current > penLow) return "BULLISH";
  return null;
}
...
export function detectBOS(highs: number[], lows: number[]): "BULLISH" | "BEARISH" | null {
  if (!highs?.length || !lows?.length || highs.length < 6 || lows.length < 6) return null;
...
export function detectCHoCH(highs: number[], lows: number[]): "BULLISH" | "BEARISH" | null {
  if (!highs?.length || !lows?.length || highs.length < 8 || lows.length < 8) return null;
